...
struct zl3073x_ptp {
	...
	struct ptp_pin_desc	pins[ZL3073X_NUM_OUTPUT_PINS];

	u16			perout_mask;
};

#define ptp_to_priv(_ptp) container_of((_ptp), struct zl3073x_ptp, info)

#define ZL3073X_PTP_IS_N_PIN(_pin) ((_pin) % 1)
#define ZL3073X_PTP_IS_P_PIN(_pin) !ZL3073X_PTP_IS_N_PIN(_pin)

static int zl3073x_gettime64(struct zl3073x_ptp *zlptp, struct timespec64 *ts,
			     unsigned int cmd)
{
	...
	zl3073x_write_dpll_tod_ctrl(zldev, zlptp->id, ctrl);
	...
	rc = zl3073x_read_dpll_tod_sec(zldev, zlptp->id, &sec);
	if (rc)
		return rc;
	rc = zl3073x_read_dpll_tod_ns(zldev, zlptp->id, &nsec);
	if (rc)
		return rc;
...
{
	struct zl3073x_ptp *zlptp = ptp_to_priv(ptp);

	/* Take device lock */
	guard(zl3073x)(zlptp->mfd);

	return zl3073x_gettime64(zlptp, ts, DPLL_TOD_CTRL_CMD_READ);
}

static int zl3073x_settime64(struct zl3073x_ptp *zlptp,
			     const struct timespec64 *ts, unsigned int cmd)
{
	...
	/* Write the value */
	rc = zl3073x_write_dpll_tod_sec(zldev, zlptp->id, ts->tv_sec);
	if (rc)
		return rc;
	rc = zl3073x_write_dpll_tod_ns(zldev, zlptp->id, ts->tv_nsec);
	if (rc)
		return rc;

	/* Issue the write command */
	tod_ctrl = DPLL_TOD_CTRL_SEM | FIELD_PREP(DPLL_TOD_CTRL_CMD, cmd);
	rc = zl3073x_write_dpll_tod_ctrl(zldev, zlptp->id, tod_ctrl);
				 ...
				 const struct timespec64 *ts)
{
	...
	return zl3073x_settime64(zlptp, ts, DPLL_TOD_CTRL_CMD_WRITE_NEXT_1HZ);
}
...
static int zl3073x_ptp_adjphase(struct ptp_clock_info *ptp, s32 delta)
{
	...
	zl3073x_write_dpll_tie_ctrl_mask(zldev, BIT(zlptp->id));
	...
	zl3073x_write_dpll_tie_data(zldev, zlptp->id,
...
static int zl3073x_steptime(struct zl3073x_ptp *zlptp, const s64 delta)
{
	struct zl3073x_dev *zldev = zlptp->mfd;
	 ...
	 */
	rc = zl3073x_read_output_ctrl(zldev, __ffs(zlptp->perout_mask),
				      &output_ctrl);
	if (rc)
		return rc;
	...
	rc = zl3073x_write_output_phase_step_mask(zldev, zlptp->perout_mask);
	if (rc)
		return rc;

	/* Start the phase adjustment on the output pin and also on the ToD */
	phase_step_ctrl = OUTPUT_PHASE_STEP_CTRL_TOD_STEP;
	phase_step_ctrl |= FIELD_PREP(OUTPUT_PHASE_STEP_CTRL_DPLL, zlptp->id);
...
static int zl3073x_ptp_wait_sec_rollover(struct zl3073x_ptp *zlptp)
{
	...
	do {
		/* Check that the semaphore is clear */
		rc = zl3073x_wait_clear_bits(zlptp->mfd, dpll_tod_ctrl,
					     DPLL_TOD_CTRL_SEM, zlptp->id);
		if (rc)
			return rc;

		/* Read the time */
		rc = zl3073x_gettime64(zlptp, &ts,
...
static int zl3073x_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	if (delta >= NSEC_PER_SEC || delta <= -NSEC_PER_SEC) {
		/* wait for rollover */
		rc = zl3073x_ptp_wait_sec_rollover(zlptp);
		if (rc)
			return rc;
	...
	return zl3073x_steptime(zlptp, delta_sub_sec_in_ns);
}
...
static int zl3073x_ptp_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	...
	return zl3073x_write_dpll_df_offset(zldev, zlptp->id, ref);
}
...
static int zl3073x_ptp_perout_disable(struct zl3073x_ptp *zlptp,
				      struct ptp_perout_request *perout)
{
	...
	pin = ptp_find_pin(zlptp->clock, PTP_PF_PEROUT, perout->index);
	if (pin == -1 || pin >= ZL3073X_NUM_OUTPUT_PINS)
		return -EINVAL;
...
static int zl3073x_ptp_perout_enable(struct zl3073x_ptp *zlptp,
				     struct ptp_perout_request *perout)
{
	struct zl3073x_dev *zldev = zlptp->mfd;
			      ...
			      struct ptp_clock_request *rq, int on)
{
	...
	if (on)
		rc = zl3073x_ptp_perout_enable(zlptp, &rq->perout);
	else
		rc = zl3073x_ptp_perout_disable(zlptp, &rq->perout);
...
static struct ptp_clock_info zl3073x_ptp_clock_info = {
	...
	.n_per_out	= ZL3073X_NUM_OUTPUT_PINS,
	.n_ext_ts	= ZL3073X_NUM_OUTPUT_PINS,
	.n_pins		= ZL3073X_NUM_OUTPUT_PINS,
};

static int zl3073x_ptp_init(struct zl3073x_ptp *zlptp)
{
	int i;

	for (i = 0; i < ZL3073X_NUM_OUTPUT_PINS; i++) {
		struct ptp_pin_desc *p = &zlptp->pins[i];
	...
	zlptp->info = zl3073x_ptp_clock_info;
	zlptp->info.pin_config = zlptp->pins;
	zlptp->clock = ptp_clock_register(&zlptp->info, zlptp->dev);
	if (IS_ERR(zlptp->clock))
		return PTR_ERR(zlptp->clock);

	return 0;
}

static bool zl3073x_ptp_dpll_nco_mode(struct zl3073x_ptp *zlptp)
{
	u8 mode, mode_refsel;
	int rc;

	rc = zl3073x_read_dpll_mode_refsel(zlptp->mfd, zlptp->id, &mode_refsel);
	if (rc)
		return false;
...
static int zl3073x_ptp_probe(struct platform_device *pdev)
{
	...
	zlptp->id = pdev->mfd_cell->id;

	scoped_guard(zl3073x, zlptp->mfd) {
		if (!zl3073x_ptp_dpll_nco_mode(zlptp))
			return -ENODEV;

		rc = zl3073x_ptp_init(zlptp);
		if (rc)
			return rc;
	}
...
static void zl3073x_ptp_remove(struct platform_device *pdev)
{
	...
	if (!IS_ERR_OR_NULL(zlptp->clock))
		ptp_clock_unregister(zlptp->clock);
}
