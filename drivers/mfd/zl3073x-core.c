...
static int zl3073x_input_state_fetch(struct zl3073x_dev *zldev, u8 index)
{
...
static int zl3073x_synth_state_fetch(struct zl3073x_dev *zldev, u8 index)
{
	...
	/* Extract and store DPLL channel where the synth is connected to */
	zldev->synth[index].dpll = FIELD_GET(SYNTH_CTRL_DPLL_SEL, synth_ctrl);

	dev_dbg(zldev->dev, "Synth %u is connected to DPLL channel %u\n", index,
